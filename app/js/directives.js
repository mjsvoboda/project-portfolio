...
var ppAppDirectives = angular.module('ppAppDirectives', []);

ppAppDirectives.directive('ppDashboard', function() {
	return {
	    restrict: 'E',
	    scope: {
	        val: '=data'
	    ...
	    link: function (scope, element, attrs) {
	    	...
	    	scope.$watch('val', function (newVal, oldVal) {
			    ...
			    newVal.forEach(function(d){d.total=d.freq["Cancelled"] + d.freq["Completed"] + d.freq["Hard Pipeline"] + d.freq["Ongoing"] + d.freq["Operationally Completed"] + d.freq["Soft Pipeline"]; });
			    ...
			    function histoGram(fD){
			        ...
			        var hGsvg = d3.select(element[0]).append("svg")
...
//			            .on("mouseout",mouseout);// mouseout is defined below.
			        ...
			        function mouseover(d){  // utility function to be called on mouseover.
			            ...
			            var st = newVal.filter(function(s){ return s.Region == d[0];})[0],
			    ...
			    function pieChart(pD){
			        ...
			        function mouseover(d){
			            ...
			            hG.update(newVal.map(function(v){ 
...
//			    var tF = ['Soft Pipeline', 'Hard Pipeline', 'Ongoing', 'Operationally Completed', 'Completed', 'Cancelled'].map(function(d){ 
			    ...
			    var tF = ['Hard Pipeline', 'Soft Pipeline', 'Ongoing', 'Operationally Completed', 'Completed', 'Cancelled'].map(function(d){
			        return {type:d, freq: d3.sum(newVal.map(function(t){ return t.freq[d];}))}; 
			    });
			    
			    // calculate total frequency by Region for all segment.
			    var sF = newVal.map(function(d){return [d.Region,d.total];});
